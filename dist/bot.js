...
exports.clickStartBlock = clickStartBlock;
exports.openTipoCasoField = openTipoCasoField;
exports.selectAllTipoCasoOptions = selectAllTipoCasoOptions;
exports.clickStartCaseHandlingBlock = clickStartCaseHandlingBlock;
exports.selectStartCaseHandlingAction = selectStartCaseHandlingAction;
exports.clickAguardandoAtendimentoStatusBlock = clickAguardandoAtendimentoStatusBlock;
exports.selectAguardandoAtendimentoStatus = selectAguardandoAtendimentoStatus;
exports.updateAllEmAtendimentoAtivoBlocks = updateAllEmAtendimentoAtivoBlocks;
exports.updateAllPresidentePendenteEmAtendimentoBlocks = updateAllPresidentePendenteEmAtendimentoBlocks;
exports.updateAllAguardandoAtendimentoBlocks = updateAllAguardandoAtendimentoBlocks;
exports.updateAllPresidenteAguardandoRespostaPacienteBlocks = updateAllPresidenteAguardandoRespostaPacienteBlocks;
exports.updateAllPresidenteConfirmacaoAguardandoAcaoBlocks = updateAllPresidenteConfirmacaoAguardandoAcaoBlocks;
exports.updateAllPresidenteAgendadoBlocks = updateAllPresidenteAgendadoBlocks;
exports.updateAllPresidenteContatoAtivoBlocks = updateAllPresidenteContatoAtivoBlocks;
exports.updateAllFinalizadoBlocks = updateAllFinalizadoBlocks;
exports.updateAllPresidenteNavegacaoPesquisaBlocks = updateAllPresidenteNavegacaoPesquisaBlocks;
...
const notAuthenticatedMessage = "Sessao nao autenticada. Abra o Chrome com remote debugging, faca login no cVortex e rode o bot novamente.";
...
async function connectToExistingChrome() {
    const config = getConfig();
    try {
        (0, utils_js_1.info)(`Conectando ao Chrome existente em ${config.cdpUrl}`);
        const browser = await playwright_1.chromium.connectOverCDP(config.cdpUrl, {
            slowMo: config.slowMoMs
        });
        const context = browser.contexts()[0] ?? await browser.newContext();
        const page = await context.newPage();
        await page.bringToFront();
        (0, utils_js_1.ok)("Conectado ao Chrome existente");
        return { browser, page };
    }
    catch (error) {
        throw new Error(`Nao foi possivel conectar ao Chrome em ${config.cdpUrl}. Abra o Chrome com --remote-debugging-port=9222 e rode novamente. Detalhe: ${error instanceof Error ? error.message : String(error)}`);
    }
}
async function openWorkflow(page, workflowUrl) {
    const config = getConfig();
    await pauseIfEnabled("abrir workflow");
    (0, utils_js_1.info)(`Abrindo workflow: ${workflowUrl}`);
    await page.bringToFront();
...
async function ensureAuthenticated(page) {
    (0, utils_js_1.info)("Verificando sessao autenticada");
    const loginSignals = [
    ...
    (0, utils_js_1.ok)("Sessao autenticada");
}
async function clickStartBlock(page) {
    await pauseIfEnabled('clicar no bloco "Iniciar"');
    (0, utils_js_1.info)("Procurando bloco Iniciar");
    const candidates = [
    ...
    if (foundCount > 1) {
        (0, utils_js_1.info)(`Foram encontrados ${foundCount} elementos visiveis com texto Iniciar. O primeiro sera usado.`);
    }
    if (!startBlock) {
...
async function openTipoCasoField(page) {
    ...
    (0, utils_js_1.info)("Procurando campo Tipo de Caso");
    ...
    const nearFieldCandidates = [
        label.locator("xpath=ancestor::*[self::label or self::div or self::section][1]//input | ancestor::*[self::label or self::div or self::section][1]//button | ancestor::*[self::label or self::div or self::section][1]//*[@role='combobox']"),
...
async function selectAllTipoCasoOptions(page) {
    ...
    (0, utils_js_1.info)("Selecionando todas as opcoes");
    ...
    for (let iteration = 0; iteration < 30; iteration += 1) {
        const options = page.locator("[role='option'], [role='menuitemcheckbox'], label:has(input[type='checkbox']), input[type='checkbox']");
...
async function ensureTipoCasoDropdownOpen(page) {
...
async function clickStartCaseHandlingBlock(page) {
    await clickWorkflowNode(page, /StarCase|StartCaseHandling/i, "StarCase");
    await waitForPropertiesPanel(page, /Propriedades de A/i);
}
async function selectStartCaseHandlingAction(page) {
    await selectPropertiesOption(page, "StartCaseHandling", "acao");
}
async function clickAguardandoAtendimentoStatusBlock(page) {
    await clickStatusNode(page, /Aguardando Atendimento/i, "Aguardando Atendimento");
}
async function selectAguardandoAtendimentoStatus(page) {
    await selectPropertiesOption(page, "Aguardando Atendimento", "status");
}
async function updateAllEmAtendimentoAtivoBlocks(page) {
    await updateAllStatusBlocks(page, { includes: ["Em Atendimento Ativo"] }, "Em Atendimento Ativo", "em-atendimento-ativo");
}
async function updateAllPresidentePendenteEmAtendimentoBlocks(page) {
    await updateAllStatusBlocks(page, { includes: ["Presidente Prudente", "Em Atendimento"], excludes: ["Ativo"] }, "Em Atendimento", "presidente-pendente-em-atendimento");
}
async function updateAllAguardandoAtendimentoBlocks(page) {
    await updateAllStatusBlocks(page, { includes: ["Aguardando Atendimento"], excludes: ["CS Aguardando Atendimento"] }, "Aguardando Atendimento", "aguardando-atendimento");
}
async function updateAllPresidenteAguardandoRespostaPacienteBlocks(page) {
    await updateAllStatusBlocks(page, { includes: ["Presidente Prudente", "Aguardando Resposta Paciente"] }, "Aguardando Resposta Paciente", "aguardando-resposta-paciente");
}
async function updateAllPresidenteConfirmacaoAguardandoAcaoBlocks(page) {
    await updateAllStatusBlocks(page, { includes: ["Presidente Prudente", "Confirmacao Aguardando Acao"] }, "Confirmacao Aguardando Acao", "confirmacao-aguardando-acao");
}
async function updateAllPresidenteAgendadoBlocks(page) {
...
async function updateAllPresidenteNavegacaoPesquisaBlocks(page) {
    await updateAllStatusBlocks(page, { includes: ["Presidente Prudente", "Navegacao BOT Pesquisa de Satisfacao"] }, "Navegacao Pesquisa de Satisfacao", "navegacao-pesquisa-satisfacao");
}
async function updateAllTabulacaoSubcategoriaBlocks(page) {
    await updateAllStatusBlocks(page, { includes: ["Tabulacao Subcategoria"] }, "Tabulacao Subcategoria", "tabulacao-subcategoria");
}
async function updateAllTabulacaoCondutaBlocks(page) {
    await updateAllStatusBlocks(page, { includes: ["Tabulacao Conduta"] }, "Tabulacao Conduta", "tabulacao-conduta");
}
...
async function saveWorkflowOnly(page) {
    (0, utils_js_1.info)("Salvando workflow sem publicar");
...
async function assertNoPublishClickTarget(page) {
...
function hasPublishText(value) {
    const normalized = normalizeText(value);
    return normalized.includes("publicar") ||
        normalized.includes("publish") ||
        normalized.includes("publicacao");
}
async function updateAllStatusBlocks(page, blockText, optionName, screenshotPrefix) {
    await updateAllBlocks(page, "status", blockText, optionName, "status", screenshotPrefix);
}
async function updateAllBlocks(page, kind, blockText, optionName, fieldName, screenshotPrefix) {
...
async function clickWorkflowNodeById(page, kind, nodeId, debugName) {
    await clickWorkflowNode(page, /.*/, debugName, page.locator(`${nodeSelector(kind)}[data-id="${nodeId}"]`));
}
async function extractClinicNameFromWorkflowTitle(page) {
...
async function selectBotFlowPesquisaSatisfacao(page, clinicName) {
    (0, utils_js_1.info)("Procurando campo Fluxo de bot");
    const input = await findEnabledBotFlowField(page);
    (0, utils_js_1.ok)("Campo Fluxo de bot encontrado");
    ...
    (0, utils_js_1.info)(`Valor atual do Fluxo de bot: ${currentValue || "vazio"}`);
    (0, utils_js_1.info)(`Clinica usada para filtro: ${selectedClinicName || "(nao identificada)"}`);
    if (isBotFlowAlreadyCorrect(currentValue, selectedClinicName)) {
        (0, utils_js_1.ok)(`Fluxo de bot ja esta correto: ${currentValue}`);
        return;
    }
...
async function waitForFieldEnabled(page, fieldName, timeoutMs = getCurrentTiming().waitForFieldEnabledTimeoutMs, intervalMs = 1000) {
    ...
    while (Date.now() <= deadline) {
        (0, utils_js_1.info)(`${firstCheck ? "Verificando se" : "Verificando novamente"} campo ${fieldName} esta habilitado`);
        firstCheck = false;
        ...
        if (input && await input.isVisible().catch(() => false) && await input.isEnabled().catch(() => false)) {
            (0, utils_js_1.ok)(`Campo ${fieldName} habilitado`);
            return input;
        }
        (0, utils_js_1.info)(`Campo ${fieldName} ainda esta desabilitado, aguardando ${intervalMs}ms`);
...
async function findPropertiesInputByFieldName(page, fieldName) {
    const properties = page.locator(".properties");
...
async function findVisibleNormalizedTextInProperties(page, text) {
...
async function findVisibleTextInProperties(page, text, screenshotOnError = true) {
...
function isReasonableBotFlowOptionText(text) {
    const normalized = normalizeText(text);
...
async function getWorkflowNodeIds(locator) {
    const count = await locator.count().catch(() => 0);
    const ids = [];
    for (let index = 0; index < count; index += 1) {
...
async function clickStatusNode(page, text, debugName, index = 0) {
...
async function clickStatusNodeById(page, nodeId, debugName) {
    await clickWorkflowNode(page, /.*/, debugName, page.locator(`.react-flow__node-status_function[data-id="${nodeId}"]`));
    await waitForPropertiesPanel(page, /Propriedades de Status/i);
}
async function clickWorkflowNode(page, text, debugName, baseLocator = page.locator(".react-flow__node"), index = 0) {
    await pauseIfEnabled(`clicar no bloco "${debugName}"`);
    (0, utils_js_1.info)(`Procurando bloco ${debugName}`);
    await page.keyboard.press("Escape").catch(() => undefined);
    ...
    (0, utils_js_1.info)(`Clicando bloco ${debugName}`);
...
async function selectPropertiesOption(page, optionName, fieldName) {
    ...
    (0, utils_js_1.ok)(`${fieldName} ${optionName} selecionado`);
}
async function selectPropertiesAutocompleteOption(page, optionName, fieldName) {
...
async function listOptionTexts(options) {
...
function findBestPesquisaSatisfacaoOption(options, clinicName) {
    const normalizedClinic = clinicName ? normalizeText(clinicName) : "";
...
function isPesquisaSatisfacaoSearch(value) {
    const normalized = normalizeText(value);
    return normalized.includes("pesquisa") && normalized.includes("satisfacao");
}
async function assertPropertiesContainSelection(page, expected, errorMessage) {
    if (!(await isPropertiesSelectionCorrect(page, expected))) {
        throw new Error(errorMessage);
    }
}
async function isPropertiesSelectionCorrect(page, expected) {
    const propertiesText = normalizeText(await page.locator(".properties").innerText().catch(() => ""));
    const inputValues = await page.locator(".properties input").evaluateAll((inputs) => inputs
        .map((input) => input instanceof HTMLInputElement ? input.value : "")
        .filter(Boolean)
        .join(" ")).catch(() => "");
    return normalizeForMatch(`${propertiesText} ${inputValues}`).includes(normalizeForMatch(expected));
}
async function isPropertiesSelectionCorrectForField(page, expected, fieldName) {
    ...
    if (normalizedField === "acao" && shouldValidateActionExactly(expected)) {
        ...
        if (normalizedCurrent === normalizedExpected) {
            (0, utils_js_1.ok)(`Campo Acao validado como: ${currentValue || expected}`);
            return true;
        }
        return false;
    }
...
async function getPropertiesCurrentValue(page) {
    const inputValues = await page.locator(".properties input").evaluateAll((inputs) => inputs
        .map((input) => input instanceof HTMLInputElement ? input.value : "")
        .filter(Boolean)
        .join(" ")).catch(() => "");
...
async function selectAllOpenAutocompleteOptions(page, listbox) {
    const seen = new Set();
    let alreadyChecked = 0;
    let checkedNow = 0;
    let unchangedIterations = 0;
...
async function assertTipoCasoHasSelection(page) {
...
async function saveChangesIfNeeded(page, validationInfo) {
    ...
    if (validationInfo.isFieldCorrect) {
        (0, utils_js_1.ok)(`Campo ${validationInfo.fieldName} preenchido com: ${validationInfo.expectedValue}`);
    }
    else {
...
async function takeScreenshot(page, filename) {
    ...
    const fullPath = node_path_1.default.join(utils_js_1.screenshotsDir, filename);
    await page.screenshot({ path: fullPath, fullPage: true });
    (0, utils_js_1.ok)(`Screenshot salvo em ./screenshots/${filename}`);
}
async function dumpVisibleTextsForDebug(page) {
    (0, utils_js_1.info)("Textos visiveis encontrados na pagina para debug:");
...
async function dumpVisiblePanelTexts(page) {
    (0, utils_js_1.info)("Textos visiveis no painel direito:");
...
async function pauseIfEnabled(message) {
    const config = getConfig();
...
async function finishBrowserConnection(browser) {
    const config = getConfig();
    if (config.keepOpen) {
        (0, utils_js_1.info)("Navegador mantido aberto para conferencia visual");
        return;
    }
    await browser.close();
}
async function findFirstVisibleInLocators(locators) {
    for (const locator of locators) {
        const item = await (0, utils_js_1.firstVisible)(locator);
        if (item) {
            return item;
        }
    }
...
async function countVisibleInLocators(locators) {
    let total = 0;
    for (const locator of locators) {
        const count = await locator.count().catch(() => 0);
        for (let index = 0; index < count; index += 1) {
...
async function selectAllNativeSelectOptions(select) {
    ...
    (0, utils_js_1.ok)(`Total de opcoes encontradas: ${values.length}`);
    (0, utils_js_1.ok)(`Opcoes ja marcadas: ${alreadyChecked}`);
    (0, utils_js_1.ok)(`Opcoes marcadas agora: ${Math.max(values.length - alreadyChecked, 0)}`);
}
async function tryClickSelectAllOption(page) {
...
async function isOptionSelected(option) {
...
async function scrollDropdownOrPanel(page) {
    const containers = [
...
async function scrollRightPanelToBottom(page) {
...
function matchesText(normalizedText, matcher) {
...
function shouldSelectExactIgnoringNumber(optionName) {
    return normalizeActionText(optionName) === "contato ativo livre";
}
function shouldValidateActionExactly(optionName) {
    return normalizeActionText(optionName) === "contato ativo livre";
}
function assertSelectedOptionAllowed(expectedValue, selectedText, matchMode) {
    ...
    if (selected !== expected) {
        throw new AutomationError(`Protecao acionada: tentativa de selecionar opcao errada. Esperado: ${expectedValue} | Encontrado: ${selectedText}`, {
            fieldName: "Acao",
            expectedValue
        });
    }
